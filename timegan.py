...
def timegan (ori_data, parameters):
  ...
  def embedder (X, T):
    ...
    with tf.compat.v1.variable_scope("embedder", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      H = tf.contrib.layers.fully_connected(e_outputs, hidden_dim, activation_fn=tf.compat.v1.nn.sigmoid)     
    return H
  ...
  def recovery (H, T):   
    ...
    with tf.compat.v1.variable_scope("recovery", reuse = tf.compat.v1.AUTO_REUSE):       
      ...
      X_tilde = tf.contrib.layers.fully_connected(r_outputs, dim, activation_fn=tf.compat.v1.nn.sigmoid) 
    return X_tilde
  ...
  def generator (Z, T):  
    ...
    with tf.variable_scope("generator", reuse = tf.AUTO_REUSE):
      ...
      E = tf.contrib.layers.fully_connected(e_outputs, hidden_dim, activation_fn=tf.compat.v1.nn.sigmoid)     
    return E
  ...
  def supervisor (H, T): 
    ...
    with tf.variable_scope("supervisor", reuse = tf.AUTO_REUSE):
      ...
      S = tf.contrib.layers.fully_connected(e_outputs, hidden_dim, activation_fn=tf.compat.v1.nn.sigmoid)     
    return S
  ...
  # 3. Two Momments
  G_loss_V1 = tf.reduce_mean(tf.abs(tf.sqrt(tf.compat.v1.nn.moments(X_hat,[0])[1] + 1e-6) - tf.sqrt(tf.compat.v1.nn.moments(X,[0])[1] + 1e-6)))
  G_loss_V2 = tf.reduce_mean(tf.abs((tf.compat.v1.nn.moments(X_hat,[0])[0]) - (tf.compat.v1.nn.moments(X,[0])[0])))
